...
const FilterButton = () => {
	...
	const handleViewClick = (title, videoUrl) => {
		const card = scrollRef.current.querySelector(
			`[data-title="${title}"][data-url="${videoUrl}"]`
		);
		if (card) {
	...
	const filteredData = data.filter(
		(item) =>
			...
			item.subtitle.toLowerCase().includes(filterValue.toLowerCase())
	);

	return (
		<div>
			{/* Label and input for the filter */}
			<label htmlFor="filter" style={{ paddingLeft: "1rem" }}>
				...
				{filteredData.map((item) => (
					...
					>
						<Card.Body>
							...
							<Button
								variant="primary"
								onClick={() => handleViewClick(item.title, item.videoUrl)}
							>
								Find
							</Button>
						</Card.Body>
